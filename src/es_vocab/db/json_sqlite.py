...
async def create_tables():
    UniverseTerm.__table_args__ = (sa.Index('tmp_idx', UniverseTerm.__table__.c.json_content['name']))
    UniverseTerm.__table_args__ = (sa.Index('json_content_idx', UniverseTerm.__table__.c.json_content['localisation']['city']))
...
async def search():
     async with AsyncSession(engine) as session:
        ...
        statement = select(UniverseTerm).where(UniverseTerm.json_content['name'] == '"ipsl"')
